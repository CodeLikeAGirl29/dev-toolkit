...
//
//Logarithmic time complexity (O(log n)) is the next best thing after constant time. While logarithmic complexity algorithms do take longer with larger inputs, run time increases slowly. For instance, if myLogRunTimeAlgo takes 1 second to complete with an input of size 10, when we increase our input by 10x to 100, the run time only grows to 2 seconds. When we increase the input size to 1000, the run time only grows to 3 seconds.
//
//It is also characteristic of logarithmic algorithms that they cut the problem size in half each round through.
//
//The following function howManyLessThanNitems has logarithmic run time complexity. It takes a sorted array of integers (arr) and a less than value (n) as arguments, and returns the number of elements from the array whose value is less than n. So howManyLessThanNitems([1, 2, 3, 4], 3) should be 2 because 1 and 2 are both less than 3.
